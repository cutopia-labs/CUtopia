...
export const logout = async () => {
  await axios.get(
    'https://cusis.cuhk.edu.hk/psp/CSPRD/EMPLOYEE/HRMS/?cmd=logout'
  );
...
export const login = async (
  ...
  const SAMLResponse = await axios
    .post(redirectResponseUrl, jsonToForm(userData))
    .catch(errorHandler);
  ...
  const homepageRes = await axios
    .post(
      'https://cusis.cuhk.edu.hk/Shibboleth.sso/SAML2/POST',
      jsonToForm(sessionData)
    )
    .catch(errorHandler);
